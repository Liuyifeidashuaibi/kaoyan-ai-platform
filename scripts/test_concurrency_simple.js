...
async function sendApiRequest(requestId) {
  ...
  try {
    ...
    const response = await nodeFetch(`${API_BASE_URL}/api/chat/send/stream`, {
      method: 'POST',
      headers: {
...
async function runConcurrencyTest() {
  ...
  if (results.successfulRequests > 0) {
    const avgTime = results.concurrentResults
      .filter(r => r.status === 'success')
      .reduce((sum, r) => sum + r.duration, 0) / results.successfulRequests;
