...
import type { TimerSubject } from "@/app/(main)/timer/_lib/types";
...
interface SubjectSectionProps {
  ...
  onRemove: (subjectId: string) => Promise<boolean>;
}
        ...
        <CardContent className="space-y-4">
          <form
            ...
            onSubmit={(event) => {
              ...
              void onAdd(trimmed).then((subjectId) => {
                if (subjectId) {
                  setDraft("");
                }
              });
          ...
          >
            <Input
              ...
              maxLength={20}
              placeholder="输入科目名称"
              disabled={subjectLocked || saving}
              onChange={(event) => setDraft(event.target.value)}
            />
            <Button
              type="submit"
              disabled={!canAdd || subjectLocked || saving}
              className="sm:w-28"
            >
              添加
            </Button>
