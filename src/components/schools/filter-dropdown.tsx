    ...
    <div ref={ref} className={cn("relative", className)}>
      <button
        ...
        className={cn(
          ...
          active ? "font-medium text-[#111827]" : "text-muted-foreground hover:text-foreground"
        )}
      >
        {displayLabel}
                ...
                {dualColumn.leftOptions.map((opt) => (
                  <button
                    ...
                    className={cn(
                      ...
                      dualColumn.leftValue === opt.value && "font-medium text-[#111827]"
                    )}
                  >
              ...
              <div className="max-h-64 w-44 overflow-y-auto py-1">
                {dualColumn.rightOptions.map((opt) => (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-muted/50",
                      dualColumn.rightValue === opt.value && "font-medium text-[#111827]"
                    )}
                  >
                    <span className="truncate">{opt.label}</span>
              ...
              {options.map((opt) => (
                <button
                  ...
                  className={cn(
                    ...
                    value === opt.value && "font-medium text-[#111827]"
                  )}
                >
                  {opt.label}
                  {value === opt.value && <Check className="size-4 text-[#111827]" />}
                </button>
              ))}
            </div>
          )}
